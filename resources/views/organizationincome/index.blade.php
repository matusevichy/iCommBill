<div>
    @can('create', [\App\Models\OrganizationIncome::class, $organization])
        <a href="{{route('organizationincomes.create', $organization->id)}}" class="btn btn-primary"
           role="button">{{__("Add")}} </a>
    @endcan
   <table class="table table-striped table-responsive-md">
        <thead>
        <tr>
            <th>{{__('Income name')}}</th>
        ...
        <tbody>
        @foreach($organization->organizationincomes as $item)
            <tr>
                <td>{{$item->name}}</td>
                ...
                <td>
                    <div class="row">
                        <div class="col col-sm-auto px-1">
                            @can('update', $item)
                                <a href="{{route('organizationincomes.edit', $item->id)}}" class="btn btn-primary"
                                   role="button"><i title="{{__('Edit')}}" class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                            @endcan
                        </div>
                        <div class="col col-sm-auto px-1">
                            @can('delete', $item)
                                <form action="{{ route('organizationincomes.destroy', $item->id) }}" method="POST"
                                      class="form-inline">
                                    @csrf
                                    @method('DELETE')
